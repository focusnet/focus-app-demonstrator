...
module.exports = function(grunt) {
		...
		replace: {
			...
			options: {
				...
				{ 
					...
					match: 'source_header',
					replacement: 
						'<%= grunt.file.read( "resources/documentation/source-header.txt") %>'
				},
				{
					match: 'timestamp',
	...
	grunt.registerTask('build-package', function() {
		...
		grunt.task.run([
			'env:prod', 'clean:build', 'copy:all', 
			'sass:all', 'clean:sassfrombuild',
			'nggetxt_compile:all',
			'replace:all',
			'clean:contribfrombuild', 'bowercopy',
			'uglify:production',
			'clean:useless',
			'preprocess:prod'
		]);
	...
	grunt.registerTask('build-dev-live', ['build-dev', 'express', 'open:firefox', 'watch']);
